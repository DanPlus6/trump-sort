...
1. Your file **must** be named **trump-sort**. All the other files of the same language must be at its title the implementation expecificaiton. For example the C++ folder have the *trump-sort.cpp* and the *trump-sort-compile-time.cpp* that runs at compile time, just as said at its name. 
2. Your file **must** have **ONE** function called Trump Sort, for pattern matters it can be named according to the language case, so if it's PascalCase the function name is trumpSort, if it's camelCase the function name is trumpSort and so on. Take note that the language case must be followed through all code.
